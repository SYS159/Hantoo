...
LOG_NAME    = "HTD_v1_6.log"        # V1.6 로그 파일명
ASSET_FILE  = "weekly_asset.json"
POSITIONS_FILE = "positions.json"   # 현재 보유 종목 기록
SOLD_FILE = "sold_today.json"       # 💡 오늘 매도한 종목(블랙리스트) 기록
...
# 💡 [신규] 1000원 미만 동전주 매수 금지 (호가 틱 가치 리스크 차단)
MIN_PRICE = 1000
...
MIN_REAL_CHANGE_RATE = 3.0  # 시초가 대비 3% 이상 찐상승 필터 유지

EXCLUDE_KEYWORDS = ["인버스", "스팩", "ETN", "ETF", "타이거", "코덱스", "KODEX", "TIGER"]

# 💡 [신규] 거래량 허들 대폭 하향 (상승 초입 조기 탑승)
...
def execute_async_sell(code, name, qty, entry_price, trigger_price, signal):
    ...
    if success:
        ...
        # 💡 [블랙리스트 추가] 한 번 청산한 종목은 오늘 다시 보지 않음
        with sold_lock:
            sold_today.add(code)
        save_sold_today()
    ...
    while True:
        try:
            ...
            if current_date != now.date():
                with sold_lock:
                    sold_today.clear()
                save_sold_today() # 파일도 빈 깡통으로 덮어씀
