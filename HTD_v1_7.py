...
LOG_NAME       = "HTD_v1_7.log"        # V1.7 로그 파일명
WEEKLY_FILE    = "weekly_info.json"    # 💡 3개의 JSON을 하나로 통합
...
VOLUME_RATIO_EARLY = 0.05   
VOLUME_RATIO_LATE  = 0.1    
...
def clear_sold_today(reason):
    """💡 09:00, 15:30 정각에 블랙리스트를 비우는 함수"""
    with sold_lock:
        sold_today.clear()
    save_sold_today()
    logging.info(f"♻️ {reason} - 블랙리스트(sold_today) 초기화 완료")
...
# (이하 API 및 매수/매도 함수 원본과 동일하게 유지 - 분량상 생략 없이 모두 붙여넣기 하시면 됩니다)
def get_top_stocks():
    token = get_token()
    if not token: return []
...
def scanner_loop():
    ...
    current_date = ""     # 💡 [해결] UnboundLocalError 방지용 변수 초기화
    opened_cleared = False 
    closed_cleared = False
    ...
    while True:
        try:
            ...
            # 💡 [블랙리스트 초기화] 장 개장 (09:00 정각 즈음)
            if now.hour == 9 and 0 <= now.minute < 2 and not opened_cleared:
                clear_sold_today("장 개장")
                opened_cleared = True

            # 💡 [블랙리스트 초기화] 장 마감 (15:30)
            if now.hour == 15 and now.minute >= 30 and not closed_cleared:
                clear_sold_today("장 마감")
                closed_cleared = True
            ...
            for stock in stocks:
                with positions_lock: current_codes = set(positions.keys())
                with sold_lock: blacklisted_codes = set(sold_today)

                code = stock.get("mksc_shrn_iscd", "")
                name = stock.get("hts_kor_isnm", "")
                change_rate = float(stock.get("prdy_ctrt", "0"))

                if code in current_codes or code in blacklisted_codes: continue
                if any(keyword in name for keyword in EXCLUDE_KEYWORDS): continue
                if not (MIN_CHANGE_RATE <= change_rate <= MAX_CHANGE_RATE): continue
                ...
                if not current_price or current_price < MIN_PRICE or not open_price or open_price <= 0: continue

                real_change_rate = ((current_price - open_price) / open_price) * 100
                if real_change_rate < MIN_REAL_CHANGE_RATE: continue
...
if __name__ == "__main__":
    ...
    # 💡 봇을 언제 켜더라도 주간 기준값이 없다면 즉시 생성 (여기 추가!)
    init_weekly_data() 
    
    msg = f"🚀 HTD 자동매매 봇 시작 (V1.7 - JSON통합/에러픽스 적용)\n현재 감시: {len(positions)}개 / 오늘 제외: {len(sold_today)}개"
    send_discord(msg)
